...
async function run() {
    console.log("🛠️ Starting FINAL image repair...");
    ...
    for (const item of TARGET_ARTICLES) {
        ...
        // Just find </p>, split, and reconstruct. No fancy HTML parsing.

        // Split strictly by the closing tag (case insensitive not needed for script gen usually, but...)
        // We trim to be safe.
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            // Skip empty parts at end
            if (i === parts.length - 1 && part.trim() === '') continue;

            newContent += part + '</p>';

            // Inject after paragraphs based on interval
...
<img src="${imgUrl}" alt="${altText}" class="w-full h-auto rounded-xl shadow-lg" loading="lazy" onError="this.onerror=null;this.src='${FALLBACK_IMAGE}';" />
        ...
        // If we missed any images (short content), append them at the end (before last P or just at end)
...
<img src="${imgUrl}" alt="${altText}" class="w-full h-auto rounded-xl shadow-lg" loading="lazy" onError="this.onerror=null;this.src='${FALLBACK_IMAGE}';" />
        ...
        const { error } = await supabase
            ...
            .update({
                ...
                // Force update timestamp to ensure cache bust if needed
                updated_at: new Date().toISOString()
            })
            .eq('id', article.id);

        if (error) console.error(`❌ Failed to update ${item.slug}:`, error);
        else console.log(`✅ Fixed ${item.slug}: Injected ${injected} images.`);
    }
}
