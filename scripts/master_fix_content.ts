...
async function run() {
    console.log("🧹 Starting Content Cleanup (Master Fix)...");
...
`;
        ...
        // 4. Update
        // Use JSON structure consistent with app, but ensure it's clean.
        // If Supabase has trouble with JSON object, we will try string again. 
        // But 'content' typically is expected to be JSON {en: ..., tr: ...}

        // The issue before was "string update" worked but caused escaping.
        // "object update" might be what avoids escaping, IF valid.

        // Let's try Object update first.
        const updatePayload = {
            en: newContent,
        ...
        if (error) {
            ...
            console.log(`✨ Updated ${item.slug} (Cleaned + ${injected} images injected)`);
            ...
            console.log(`   -> Verify: ${c} images found.`);
        }
    }
}
