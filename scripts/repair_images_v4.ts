...
async function run() {
    ...
    for (const item of TARGET_ARTICLES) {
        ...
        // DEBUG found that content is sometimes string, sometimes object?
        // Let's force it to be treated correctly.
        let contentStr = "";
        let isJson = false;
...
<img src="${imgUrl}" alt="${altText}" class="w-full h-auto rounded-xl shadow-lg" loading="lazy" onError="this.onerror=null;this.src='${FALLBACK_IMAGE}';" />
        ...
        // 4. Update
        // IMPORTANT: We must write back in the same format we found it, OR standardise to JSON if schema supports it.
        // If debug showed 'string', maybe column is TEXT? But schema says JSONB usually.
        // However, Supabase/PG can auto-cast string to JSONB if valid JSON. But if it's just HTML, it's not valid JSON.
        // If column is JSONB, and we read it as string, it implies it stored a string *inside* JSONB? Or column is TEXT.

        // Let's try to update as JSON object always, as that's our app convention.
        // { en: "HTML", tr: "..." }
        ...
        if (error) {
            ...
            // Fallback: maybe column is TEXT? try sending just string?
            // console.log("Trying string update...");
            // await supabase.from('articles').update({ content: newContent }).eq('id', article.id);
        } else {
            console.log(`✅ Fixed ${item.slug}: Injected ${injected} images.`);
            // console.log("Updated data:", JSON.stringify(data).slice(0, 100));
        }
    }
}
