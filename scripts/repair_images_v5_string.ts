...
async function run() {
    console.log("🛠️ Starting FINAL V5 image repair (STRING UPDATE attempt)...");
    ...
    for (const item of TARGET_ARTICLES) {
        ...
        // Treat as string no matter what
...
<img src="${imgUrl}" alt="${altText}" class="w-full h-auto rounded-xl shadow-lg" loading="lazy" onError="this.onerror=null;this.src='${FALLBACK_IMAGE}';" />
        ...
        // TRY 1: Update as string directly. If it fails, we know for sure.
        const { error } = await supabase
            .from('articles')
            .update({
                content: newContent, // Send as STRING
        ...
        if (error) {
            console.error(`❌ Failed to update (string mode) ${item.slug}:`, error);
        } else {
            console.log(`✅ Fixed (string mode) ${item.slug}: Injected ${injected} images.`);
            ...
            const verifyContent = typeof verifyData?.content === 'string' ? verifyData.content : (verifyData?.content as any)?.en || "";
